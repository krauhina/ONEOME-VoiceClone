...
async def handle_text(update: Update, context: CallbackContext) -> None:
    ...
    voice_file_path = os.path.join(voice_dir, f'voice_message_user_{user_id}.ogg')
    print("ss1")
    await process_files(user_id, update, context)


async def process_files(user_id, update: Update, context: CallbackContext) -> None:
    text_file_path = os.path.join(text_dir, f'text_message_user_{user_id}.txt')
    voice_file_path = os.path.join(voice_dir, f'voice_message_user_{user_id}.ogg')
    voice_file_path_result = os.path.join(result_dir, f'voice_message_user_{user_id}.ogg')
    ...
    with open(voice_file_path_result, 'rb') as audio_file:  # Открываем аудиофайл в бинарном режиме
        await update.message.reply_audio(audio=audio_file, caption='Ваш аудиофайл готов! Если вам нужно озвучить другой текст, то присылайте его ниже')
